...
for j in range(len(A_list)):
    ...
    directory = "C:/DATA/"
    # directory = "E:/DATA/Particles 2,744k Isothermal~step 5000/Particle Data/"
    os.chdir(directory)
    ...
    while p < steps:
        ...
        f1 = plt.figure(1)
        V = plt.quiver(x, y, u, v, z, cmap='gnuplot_r')
        plt.colorbar(V)
        plt.gca()
        plt.xlim([0,11])
        plt.ylim([0,11]) # 18 lim for 2744
        name = "image"
        num = str(p)
        num = num.zfill(5)
        name += num + '.png'
...
########################################################################################################################
        ...
        vv = vy[q][:]
        f2 = plt.figure(2)
        Q = plt.quiver(x, y, u, v, z, cmap='gnuplot_r')
        plt.colorbar(Q)
        plt.gca()
        plt.xlim([0, 11])
        plt.ylim([0, 11])
