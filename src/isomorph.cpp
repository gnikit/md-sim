...
Isomorph::Isomorph(double RHO, double T, double Ar, vec1d T_in) {
   ...
   *  T_in is the input Temperature vector, where the isomorph is placed
   *
   *  @param RHO: Reference point density
   *  @param T: Reference point temperature
   *  @param Ar: Reference point A
   *  @param T_in: Range of temperatures for which isomorph points will be
   *               generated.
   */
  _rho_r = RHO;
  _T_r = T;
...
std::tuple<vec1d, vec1d> Isomorph::GenLine(size_t n) {
   ...
   *  or the density and A parameter in the form of a vector.
   *
